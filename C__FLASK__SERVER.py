...
def bc_make_predict():
	...
	inputs = pd.DataFrame({
		'Pain': [int(predict_request[0])],
		'Haemoptysis': [int(predict_request[1])],
		'Dyspnoea': [int(predict_request[2])],
		'Cough': [int(predict_request[3])],
		'Weakness': [int(predict_request[4])],
		'Tumor_Size': [int(predict_request[5])],
		'Diabetes_Mellitus': [int(predict_request[6])],
		'MI_6mo': [int(predict_request[7])],
		'PAD': [int(predict_request[8])],
	})
